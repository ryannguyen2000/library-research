...
function* fetch({ payload }) {
  try {
    const { error_code, data } = yield call(client().get, `${apiUrl}/payment/pay/request`, { params: payload });
    if (error_code === 0) {
      yield put({
...
function* refresh() {
  const query = yield select(state => state.accounting.expense.newExpense.query);
