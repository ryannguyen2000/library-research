...
def integral(zeroDaysBeforeRate, oneDaysBeforeRate, twoDaysBeforeRate, threeDaysBeforeRate):
    ...
    maxRate = max(r for _, r in ret)
    ret = [[t, r*zeroDaysBeforeRateT/maxRate] for t, r in ret]
...
def lastMinutesPrice(durationRates, remainTime, capacity,
    ...
    constraints = (
        ...
        {
            'type': 'ineq',
            'fun': lambda x:  constraint_2(x, maximum_price=1.0 - minimum_promotion)
        },
        {
            'type': 'ineq',
...
def main():
    ...
    parser.add_argument('-T', '--total_room', type=int, default=30,
                        help='number of rooms.')

    parser.add_argument('-a', '--available_room', type=int, default=2,
                        help='number available of rooms.')

    parser.add_argument('-H', '--hour', type=int, default=2,
                        help='time (hour) in day')
    ...
    configuration = {
        ...
        # 'hours':args['hour'],
        'capacity': args['available_room'],
    }

    price = lastMinutesPrice(
        durationRates, {'hours': args['hour']},  **configuration).x[0]

    minimum_promotion = args['minimum_promotion']
    if price > 1.0 - minimum_promotion:
        price = 1.0 - minimum_promotion

    configuration['price'] = price
    configuration['promotion'] = 1.0 - price
